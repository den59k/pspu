...
import { uploadUrl } from 'libs/constants'
...
export default function Gallery ({className, form, label, name}){
	...
	const onFileChange = (e) => {
		...
		reader.onload = async () => {
			...
			const json = await fetch(uploadUrl, { method: 'POST', headers, body: array } )
		...
		<div className={cn("gallery-control", className, form.errors.get(name) && "errored")}>
			...
			<div className="gallery-list">
				{values.map((item, index) => (
					<button key={index+item.src} className={cn(selection.has(index) && "active")} onClick={() => select(index)}>
